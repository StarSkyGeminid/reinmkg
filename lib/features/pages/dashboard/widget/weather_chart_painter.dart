...
class WeatherChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxTemp =
        data.map((e) => e.temperature!).reduce((a, b) => a > b ? a : b);
    final minTemp =
        data.map((e) => e.temperature!).reduce((a, b) => a < b ? a : b);

    final heightRatio = size.height / (maxTemp - minTemp) * 0.48;

    List<Offset> points = [];

    final y =
        size.height - ((data[0].temperature! - minTemp) * heightRatio) - 32.sp;
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      final y = size.height -
          ((data[i].temperature! - minTemp) * heightRatio) -
          32.sp;
