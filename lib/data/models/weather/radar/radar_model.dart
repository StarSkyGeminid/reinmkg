...
class RadarModel extends RadarEntity {
  ...
  factory RadarModel.fromJson(Map<String, dynamic> json) {
    ...
    final time = timeRaw != null
        ? DateFormat('yyyy-MM-dd HH:mm').tryParse(
            timeRaw.replaceAll(' UTC', ''),
            true,
          )
        : null;
    ...
    return RadarModel(
      ...
      time: time,
      file: json.containsKey('CMAX') && json['CMAX'].containsKey('file')
          ? json['CMAX']['file']
          : null,
      distance:
          json.containsKey('jarak') ? double.tryParse(json['jarak']) : null,
      code: json['kode'],
      tlc: LatLng(double.parse(json['overlayTLC'][0] as String),
          double.parse(json['overlayTLC'][1] as String)),
      brc: LatLng(double.parse(json['overlayBRC'][0] as String),
