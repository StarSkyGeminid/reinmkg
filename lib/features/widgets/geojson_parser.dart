...
class GeoJsonParser {
  ...
  Future<void> parseGeoJsonAsString(String g) async {
    try {
      // return parseGeoJson(jsonDecode(g) as Map<String, dynamic>);

      return (kReleaseMode || kProfileMode)
          ? (compute(jsonDecode, g))
              .then((value) => parseGeoJson(value as Map<String, dynamic>))
          : parseGeoJson(jsonDecode(g) as Map<String, dynamic>);
    } catch (e) {
      log.d(e);
              ...
              for (final coords in path as List<dynamic>) {
                if (pathIndex == 0) {
                  ...
                  outerRing.add(LatLng(
                      (coords[1] is double
                          ? coords[1]
                          : (coords[1] as int).toDouble()),
                      (coords[0] is double
                          ? coords[0]
                          : (coords[0] as int).toDouble())));
                } else {
                  // add it to a hole
                  hole.add(LatLng(
                      (coords[1] is double
